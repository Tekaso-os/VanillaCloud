...
if ( 
  ...
  ! empty ( $dados_usuario['senha'] ) 
	...
	$pdo_checa_user = $conexao_pdo->prepare('SELECT * FROM usuarios WHERE email = ? LIMIT 1');
	...
	if ( crypt( $dados_usuario['senha'], $fetch_usuario['senha'] ) === $fetch_usuario['senha'] ) {
		...
		$_SESSION['discord']  =  $fetch_usuario['discord'];
		...
		$_SESSION['login_erro'] = 'Senha ou email incorretos.';
	}
}
?>